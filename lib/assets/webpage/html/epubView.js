  ...
  try {
    ...
    if (cfiOrHref.startsWith('epubcfi(')) {
      ...
      if (spineItem) {
        ...
        if (fragment) {
          try {
            var doc = await spineItem.load(book.load.bind(book));
        ...
        if (!cfi) {
          // Find the first location for this spine item
          // Locations are generated sequentially for each spine item
          // We can estimate by checking spine index
          var spineIndex = spineItem.index;
          console.log('🔍 getPageFromCfi -> using spine index:', spineIndex);
          
          // Try to find first location matching this spine
          // Each location has a corresponding CFI - find ones for this spine
          var locationsArray = book.locations._locations;
          if (locationsArray && locationsArray.length > 0) {
            console.log('🔍 getPageFromCfi -> searching in', locationsArray.length, 'locations');
            // Find first location for this spine item
            for (var i = 0; i < locationsArray.length; i++) {
              var loc = locationsArray[i];
              if (typeof loc === 'string' && loc.indexOf('/6/' + (spineIndex * 2 + 2) + '!') >= 0) {
                console.log('✅ getPageFromCfi -> found location at page:', i + 1);
                return i + 1;
              }
            }
          }
          
          // Fallback: estimate based on spine index
