...
(function () {
  ...
  function onTouchEnd(e) {
    ...
    var direc = "";
    // Horizontal swipe detection (same thresholds as detectSwipe)
    if (((gs.eX - minSwipe > gs.sX) || (gs.eX + minSwipe < gs.sX)) &&
      (gs.eY < gs.sY + maxCross && gs.sY > gs.eY - maxCross)) {
  ...
  document.addEventListener('pointerup', function (e) {
    ...
    if (Math.abs(dx) >= minSwipe && dy < maxCross) {
      lastSwipeNavTime = now;
      if (dx < 0) {
        rendition.next();
      } else {
