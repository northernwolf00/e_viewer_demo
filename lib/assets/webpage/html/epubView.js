...
var useCustomSwipeGlobal = false; // Global flag for custom swipe navigation
var lastSwipeNavTime = 0; // Debounce timestamp for swipe navigation
...
function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam, customCss) {
  ...
  // Store custom swipe setting globally for iOS fallback
  useCustomSwipeGlobal = !!useCustomSwipe;
  ...
  try {
    ...
    window.flutter_inappwebview.callHandler('error', "Error opening book: " + e.message);
  }
  ...
  rendition.on("relocated", function (location) {
    ...
    // Get the current href (spine item)
  ...
  rendition.hooks.content.register((contents) => {
    ...
    // Detect and mark cover pages
    // Cover pages typically have a single large image with minimal or no text
    function detectCoverPage() {
      try {
        ...
        // Get visible text content (excluding image alt texts)
        var textNodes = [];
        ...
        // Cover detection criteria (very aggressive to catch cover images):
        // 1. Has at least one image AND very little text (less than 200 characters)
        // 2. OR: single image and less than 500 characters
        // 3. OR: page has "cover" in the href/filename
        // 4. OR: first page of the book with an image
        var isCover = false;

        // Check if href contains "cover" or "title"
        var href = contents.href || '';
        var hrefLower = href.toLowerCase();
        var hasCoverInHref = hrefLower.indexOf('cover') !== -1 ||
          hrefLower.indexOf('title') !== -1 ||
          hrefLower.indexOf('titlepage') !== -1;

        // Very aggressive detection
        if (images.length >= 1 && totalTextLength < 200) {
          isCover = true;
        } else if (images.length === 1 && totalTextLength < 500) {
          isCover = true;
        } else if (hasCoverInHref && images.length > 0) {
          isCover = true;
        } else if (images.length > 0 && images[0].offsetHeight > 400) {
          // Large image (likely a cover)
          isCover = true;
        }

        if (isCover) {
          body.classList.add('epub-cover-page');
          body.setAttribute('data-cover-page', 'true');
          ...
          body.classList.remove('epub-cover-page');
          body.removeAttribute('data-cover-page');
        }
      } catch (e) {
      }
    }

    // Run detection on load and multiple times with delays to ensure it catches the cover
    detectCoverPage();
    setTimeout(detectCoverPage, 100);
    setTimeout(detectCoverPage, 300);
    setTimeout(detectCoverPage, 500);
    setTimeout(detectCoverPage, 1000);
    setTimeout(detectCoverPage, 300);
    ...
    if (useCustomSwipe) {
      // Detect swipes on iframe document element (works on Android)
      ...
      if (el) {
        detectSwipe(el, function (el, direction) {
          ...
          var now = Date.now();
          if (now - lastSwipeNavTime < 300) return;
...
// Global swipe detection for iOS compatibility
// On iOS, touch events in iframes don't fire, so the iframe-level detectSwipe doesn't work.
// This detects swipes on the main document and triggers page navigation.
(function () {
  var gs = { sX: 0, sY: 0, eX: 0, eY: 0, active: false };
  var minSwipe = 50;
  var maxCross = 50;

  function onTouchStart(e) {
    if (!useCustomSwipeGlobal) return;
    var touch = e.touches && e.touches.length > 0 ? e.touches[0] : null;
    if (!touch) return;
    gs.sX = touch.screenX;
    gs.sY = touch.screenY;
  ...
  function onTouchEnd(e) {
    ...
    gs.active = false;
    if (!useCustomSwipeGlobal) return;
    if (typeof hasActiveSelection === 'function' && hasActiveSelection()) return;
    if (typeof rendition === 'undefined' || !rendition) return;

    // Debounce: skip if iframe-level detectSwipe already handled this
    var now = Date.now();
    if (now - lastSwipeNavTime < 300) return;

    var direc = "";
    var absDx = Math.abs(gs.eX - gs.sX);
    var absDy = Math.abs(gs.eY - gs.sY);
    // Horizontal swipe detection: must be primarily horizontal (dx > dy * 1.5)
    if (absDx >= minSwipe && absDy < maxCross && absDx > absDy * 1.5) {
      direc = gs.eX > gs.sX ? "r" : "l";
    }
  ...
  // Attach to document and window (capture phase)
  ...
  // Also use pointer events as fallback (iOS 13+)
  var pointerStart = { x: 0, y: 0, active: false };
  document.addEventListener('pointerdown', function (e) {
    if (!useCustomSwipeGlobal) return;
    if (e.pointerType === 'touch' || e.pointerType === '') {
      pointerStart.x = e.screenX;
      pointerStart.y = e.screenY;
  ...
  document.addEventListener('pointerup', function (e) {
    ...
    var dx = e.screenX - pointerStart.x;
    var dy = Math.abs(e.screenY - pointerStart.y);
    // Must be primarily horizontal (dx > dy * 1.5) to trigger page navigation
    if (Math.abs(dx) >= minSwipe && dy < maxCross && Math.abs(dx) > dy * 1.5) {
      lastSwipeNavTime = now;
      if (dx < 0) {
        rendition.next();
      } else {
...
function getCurrentLocation() {
  ...
  // Get the current href (spine item)
  var currentHref = rendition.location.start.href || '';
...
// adds highlight with given color (now as underline)
function addHighlight(cfiRange, color, opacity) {
  rendition.annotations.underline(cfiRange, {}, (e) => {
    // Highlight clicked handler (can be extended if needed)
  }, "hl", { "stroke": color, "stroke-width": "3px", "stroke-opacity": '0.8', "mix-blend-mode": "multiply", "y": "15%" });
}
...
function setFontSize(fontSize) {
  ...
  // Store font size globally
  window.currentFontSize = fontSize;

  // Get current theme colors
  var bgColor = window.currentBackgroundColor || "";
  var fgColor = window.currentForegroundColor || "";
  var css = window.currentCustomCss || {};


  // Add font-size to customCss
  css['font-size'] = fontSize + 'px';

  // Reapply theme with new font size
  updateTheme(bgColor, fgColor, css);
  rendition.reportLocation();
}

//get page information (current page and total pages)
function getPageInfo() {
  ...
  // Call length() as a function, not as a property
  ...
  if (rendition && rendition.location && rendition.location.start) {
    ...
    if (currentCfi) {
      try {
        // Get the location index from CFI (1-based index)
        var locationIndex = book.locations.locationFromCfi(currentCfi);

        if (locationIndex && locationIndex > 0) {
          currentPage = locationIndex;
        }
      } catch (e) {
      }
    }
...
//get page number from CFI or href
async function getPageFromCfi(cfiOrHref) {
  console.log('📍 getPageFromCfi CALLED -> input:', cfiOrHref);

  if (!book.locations) {
    console.log('❌ getPageFromCfi -> no book.locations');
    return null;
  }

  var totalPages = book.locations.length();
  if (!totalPages || totalPages === 0) {
    console.log('❌ getPageFromCfi -> totalPages is 0 or undefined');
    return null;
  }

  try {
    // Track CFI separately; default to null unless explicitly provided
    var cfi = null;

    // If it's already a full CFI, use it directly
    if (cfiOrHref.startsWith('epubcfi(')) {
      cfi = cfiOrHref;
      console.log('✅ getPageFromCfi -> already a CFI:', cfi);
    } else {
      // Strip fragment if present (chapter.xhtml#id)
      var parts = cfiOrHref.split('#');
      var hrefOnly = parts[0];
      var fragment = parts.length > 1 ? parts[1] : null;
      console.log('🔍 getPageFromCfi -> parsing href:', hrefOnly, 'fragment:', fragment);

      // It's an href, find the spine item (tolerate relative/absolute mismatches)
      var spineItem = book.spine.get(hrefOnly);
      if (!spineItem && book.path && typeof book.path.resolve === 'function') {
        var resolvedHref = book.path.resolve(hrefOnly);
        console.log('🔍 getPageFromCfi -> trying resolved:', resolvedHref);
        spineItem = book.spine.get(resolvedHref);
      }
      if (!spineItem && book.spine && book.spine.spineItems) {
        for (var i = 0; i < book.spine.spineItems.length; i++) {
          var item = book.spine.spineItems[i];
          if (!item || !item.href) continue;
          if (item.href === hrefOnly || item.href.endsWith('/' + hrefOnly) || hrefOnly.endsWith(item.href)) {
            console.log('✅ getPageFromCfi -> found spine item via loop index:', i);
            spineItem = item;
            break;
          }
        }
      ...
      if (spineItem) {
        console.log('✅ getPageFromCfi -> spineItem found:', spineItem.href, 'index:', spineItem.index);

        // If we have a fragment, load and find element
        if (fragment) {
          try {
            await spineItem.load(book.load.bind(book));
            var doc = spineItem.document;
            var el = (doc.querySelector ? doc.querySelector('#' + fragment) : null) || (doc.getElementById ? doc.getElementById(fragment) : null);
            if (el) {
              cfi = spineItem.cfiFromElement(el);
              console.log('✅ getPageFromCfi -> generated CFI from fragment element:', cfi);
            } else {
              console.log('⚠️ getPageFromCfi -> fragment element not found, will use spine start');
              cfi = null;
            }
            spineItem.unload();
          } catch (err) {
            console.error('❌ getPageFromCfi -> error loading spine item:', err);
            cfi = null;
          }
        }

        // If no CFI from fragment (or no fragment), use spine item start location
        if (!cfi) {
          var spineIndex = spineItem.index;
          console.log('🔍 getPageFromCfi -> using spine index:', spineIndex);

          // Find first location CFI for this spine item, then use locationFromCfi
          // so page numbers are consistent with getPageInfo()
          // Construct a CFI for the start of this spine item to get accurate page number
          // This matches the logic used in getPageInfo() which uses locationFromCfi()
          try {
            var cfiBase = '/6/' + (spineIndex * 2 + 2);
            var startCfi = 'epubcfi(' + cfiBase + '!/0)';

            var locIdx = book.locations.locationFromCfi(startCfi);

            if (locIdx != null && locIdx >= 0) {
              console.log('✅ getPageFromCfi -> constructed startCfi:', startCfi, 'returned:', locIdx);
              // locationFromCfi returns 0-based index, convert to 1-based page number
              return locIdx + 1;
            }
          } catch (e) {
            console.error('Error getting page from constructed CFI', e);
          }

          // Fallback: estimate based on spine position
          var estimatedPage = Math.floor((spineIndex / book.spine.length) * totalPages) + 1;
          console.log('⚠️ getPageFromCfi -> using estimated page:', estimatedPage);
          return estimatedPage;
        }
      } else {
        console.log('❌ getPageFromCfi -> spineItem NOT found for:', hrefOnly);
        return null;
      }
    }

    // If we have a valid CFI, try to get location from it
    if (cfi) {
      var locationIndex = book.locations.locationFromCfi(cfi);
      console.log('📍 getPageFromCfi -> locationFromCfi returned:', locationIndex);

      if (locationIndex != null && locationIndex >= 0) {
        // locationFromCfi returns 0-based index, convert to 1-based page number
        var pageNumber = locationIndex + 1;
        console.log('✅ getPageFromCfi -> RETURNING page:', pageNumber);
        return pageNumber;
      }
    }
  } catch (e) {
    console.error('❌ getPageFromCfi -> ERROR:', e);
  }

  console.log('❌ getPageFromCfi -> RETURNING null');
  return null;
}

// Get all chapter pages using the same locationFromCfi logic as getPageFromCfi
// so that chapter page numbers match the reader's current page display.
async function getAllChapterPages() {
  console.log('📍 getAllChapterPages CALLED');

  if (!book.locations || !chapters || chapters.length === 0) {
    console.log('❌ getAllChapterPages -> missing locations or chapters');
    return {};
  }

  var totalPages = book.locations.length();
  if (!totalPages || totalPages === 0) {
    console.log('❌ getAllChapterPages -> totalPages is 0 or undefined');
    return {};
  }
  ...
  for (var i = 0; i < chapters.length; i++) {
    ...
    try {
      var page = await getPageFromCfi(href);
      if (page != null && page > 0) {
        result[href] = page;
      } else {
        // Fallback: estimate based on spine position
        ...
        var spineItem = book.spine.get(hrefOnly);
        if (!spineItem) {
          for (var j = 0; j < book.spine.spineItems.length; j++) {
            var item = book.spine.spineItems[j];
            if (item.href === hrefOnly || item.href.endsWith('/' + hrefOnly) || hrefOnly.endsWith(item.href)) {
              spineItem = item;
              break;
            }
        ...
        if (spineItem) {
          var estimatedPage = Math.floor((spineItem.index / book.spine.length) * totalPages) + 1;
          result[href] = estimatedPage;
          console.log('⚠️ Chapter ' + i + ': ' + chapter.label + ' using estimate page ' + estimatedPage);
        }
      }
    } catch (e) {
      console.error('❌ Chapter ' + i + ': error getting page for ' + href, e);
    }
  }

  console.log('✅ getAllChapterPages -> mapped ' + Object.keys(result).length + ' chapters');
  return result;
}
...
function updateTheme(backgroundColor, foregroundColor, customCss) {
  ...
  if (window.flutter_inappwebview) {
    window.flutter_inappwebview.callHandler('onConsoleMessage', {
      message: '🔵 updateTheme CALLED - bg:' + backgroundColor + ' fg:' + foregroundColor + ' css:' + JSON.stringify(customCss),
      messageLevel: 1
    });
  }

  // Store current theme values globally for setFontSize
  window.currentBackgroundColor = backgroundColor;
  window.currentForegroundColor = foregroundColor;
  window.currentCustomCss = customCss;

  var rules = {};

  // Build body styles - DO NOT use flexbox centering for body
  // This would center ALL content including text chapters
  // For normal pages, use more padding to avoid filling the entire screen.
  var bodyStyles = {
    "padding": "60px 30px !important",  // Padding: 60px top/bottom, 30px left/right
    "display": "block !important",  // Ensure normal flow for text content
    "min-height": "auto !important",  // Don't force full viewport height for text
  ...
  if (backgroundColor && backgroundColor !== "" && backgroundColor !== "null") {
    bodyStyles["background"] = backgroundColor;
  }
  if (foregroundColor && foregroundColor !== "" && foregroundColor !== "null") {
    bodyStyles["color"] = foregroundColor;
  }

  // Apply font-family, font-weight, font-size from customCss to body
  if (customCss && typeof customCss === 'object') {
    if (customCss['font-family']) {
      bodyStyles["font-family"] = customCss['font-family'] + " !important";
    }
    if (customCss['font-weight']) {
      bodyStyles["font-weight"] = customCss['font-weight'] + " !important";
    }
    if (customCss['font-size']) {
      bodyStyles["font-size"] = customCss['font-size'] + " !important";
    }
  }

  if (Object.keys(bodyStyles).length > 0) {
    rules["body"] = bodyStyles;
  }

  // Apply same font properties to all text elements
  var textElements = "p, span, div, li, td, th, a, blockquote, pre, code";
  var textStyles = {};
  if (customCss && typeof customCss === 'object') {
    if (customCss['font-family']) {
      textStyles["font-family"] = customCss['font-family'] + " !important";
    }
    if (customCss['font-weight']) {
      textStyles["font-weight"] = customCss['font-weight'] + " !important";
    }
    if (customCss['font-size']) {
      textStyles["font-size"] = customCss['font-size'] + " !important";
    }
  }
  if (Object.keys(textStyles).length > 0) {
    rules[textElements] = textStyles;
  }

  // Center all images in EPUB content horizontally (for normal pages)
  rules["img"] = {
    "display": "block",
    "margin-left": "auto",
    "margin-right": "auto",
  ...
  // ONLY for cover pages - center the image vertically AND horizontally
  // This uses flexbox to center the entire page content
  rules["body.epub-cover-page"] = {
    ...
    "margin": "0 !important",
    "padding": "0 !important",  // Remove padding for cover
    "overflow": "hidden !important"  // Prevent scrolling on cover page
  };

  // Cover page image styling - ensure it fits perfectly
  rules["body.epub-cover-page img"] = {
    "max-height": "100vh !important",
  ...
  // Handle cover pages that use figure or div wrappers
  rules["body.epub-cover-page figure"] = {
    "margin": "0 !important",
    "padding": "0 !important",
    "display": "flex !important",
    "align-items": "center !important",
  ...
  rules["body.epub-cover-page div"] = {
    "display": "flex !important",
    "align-items": "center !important",
    "justify-content": "center !important"
  };

  // Remove borders/backgrounds from chapter headings
...
function detectSwipe(el, func) {
  ...
  ele.addEventListener('touchend', function (e) {
    ...
    var absDeltaX = Math.abs(swipe_det.eX - swipe_det.sX);
    var absDeltaY = Math.abs(swipe_det.eY - swipe_det.sY);
    //horizontal detection - must be primarily horizontal (dx > dy * 1.5)
    if (absDeltaX >= min_x && absDeltaY < max_y && absDeltaX > absDeltaY * 1.5) {
